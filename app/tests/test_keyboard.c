...
static void test_tap_on_known_key_generates_down_up_pair(void) {
    ...
    int n = keyboard_handle_tap(keyboard, x, y, events, &visual_changed);
...
static void test_tap_outside_any_key_returns_zero_events(void) {
    ...
    int n = keyboard_handle_tap(keyboard, -5, -5, events, &visual_changed);
    assert(n == 0);
    assert(visual_changed == false);
...
static void test_sticky_shift_uppercases_next_letter_then_disarms(void) {
    ...
    int n = keyboard_handle_tap(keyboard, sx, sy, events, &visual_changed);
    assert(n == 0);
    ...
    n = keyboard_handle_tap(keyboard, qx, qy, events, &visual_changed);
    assert(n == 2);
...
static void test_sticky_ctrl_wraps_control_l_around_letter(void) {
    ...
    int n = keyboard_handle_tap(keyboard, ctrl_x, ctrl_y, events, &visual_changed);
    assert(n == 0);
    assert(visual_changed == true);
    assert(keyboard_key_view(keyboard, ctrl_index).highlighted == true);

    visual_changed = false;
    n = keyboard_handle_tap(keyboard, cx, cy, events, &visual_changed);
    assert(n == 4);
...
static void test_ctrl_and_shift_together_wrap_uppercase_letter(void) {
    ...
    keyboard_handle_tap(keyboard, sx, sy, events, &visual_changed);
    keyboard_handle_tap(keyboard, ctrl_x, ctrl_y, events, &visual_changed);
    assert(keyboard_key_view(keyboard, shift_index).highlighted == true);
    assert(keyboard_key_view(keyboard, ctrl_index).highlighted == true);

    visual_changed = false;
    int n = keyboard_handle_tap(keyboard, cx, cy, events, &visual_changed);
    assert(n == 4);
    assert(events[0].keysym == KS_CONTROL_L && events[0].down == true);
...
static void test_page_switch_changes_keys_at_same_position(void) {
    ...
    int n = keyboard_handle_tap(keyboard, px, py, events, &visual_changed);
    assert(n == 0);
    assert(visual_changed == true);
    ...
    visual_changed = false;
    n = keyboard_handle_tap(keyboard, ax, ay, events, &visual_changed);
    assert(n == 0);
    assert(visual_changed == true);
